...
@timeutil.AfterDelta
def log_task_state(in_tasks, in_run, in_done):
    log.debug('Manager: tasks [%s] running [%s] done [%s]',
              in_tasks, in_run, in_done)
...
class Manager(object):
    ...
    """
    def __init__(self, max_threads, thread_timeout, stdout_dir, stderr_dir):
        self.limit = max_threads
        self.timeout = thread_timeout
        self.outdir = stdout_dir
        self.errdir = stderr_dir
    ...
    def run(self):
        ...
        try:
            ...
            log.debug('Manager: tasks [%s] running [%s] done [%s]',
                      len(self.tasks), len(self.running), len(self.done))
            try:
                ...
                while self.running or self.tasks:
                    ...
                    # because we are in a loop and there is no need to
                    # dump the exact same data out every few miliseconds
                    # added a decorator that will cause the log to
                    # happen every minute by default
                    log_task_state(len(self.tasks), len(self.running),
                                   len(self.done))
    ...
    def finished(self, task):
        ...
        log.trace('Task Finished: %s', task)
        self.done.append(task)
        if task.callback is not None:
            try:
                log.trace('Attempting callback for task...')
                task.callback(task.torrent, task.media,
                              tuple(task.get_results()))
            except Exception as err:
                # could be lots of reasons; we'll get the info from
                # statuses return and do it long hand.
                log.debug('Call back attempt failed...[%s]', err)
                pass
...
class Writer(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if data == self.OPEN:
                ...
                set_cloexec(self.files[filename])
            else:
    ...
    def open_files(self, host):
        ...
        if self.outdir or self.errdir:
            ...
            if count:
                filename = '{0}.{1}'.format(host, count)
            else:
                filename = host
            if self.outdir:
